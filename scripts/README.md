...
| `run_coldstart.py` | cold-start: K=5 leave-one-fold-out on Set 1 — donor chain over the other stations, then zero-shot + fine-tune on each held-out station |
| `run_stability_samples.py` | Set 2 stability: independent random 50+50 4G/5G samples (disjoint from the primary 100), base campaign per sample |
