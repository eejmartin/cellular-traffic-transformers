...
class Model_backbone(nn.Module):
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        # denorm
        # denormalize the full patch-rounded output (pred_patch_num * patch_len
        # steps); the Model wrapper slices to pred_len afterwards. Using
        # z.shape[-1] instead of pred_len keeps this valid when pred_len is
        # not a multiple of patch_len (identical values under broadcasting
        # for the multiples, so existing results are unaffected).
        z = z * (std[:, :, 0].unsqueeze(2).repeat(1, 1, z.shape[-1]))
        z = z + (mean[:, :, 0].unsqueeze(2).repeat(1, 1, z.shape[-1]))
